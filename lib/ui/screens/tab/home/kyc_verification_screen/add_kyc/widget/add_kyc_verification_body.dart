...
import 'package:digitifyze/ui/screens/tab/home/kyc_verification_screen/add_kyc/add_kyc_verification_screen_controller.dart';
...
class AddKycVerificationBodyScreen extends ConsumerStatefulWidget {
  final int existLeadId;
  const AddKycVerificationBodyScreen({super.key, required this.existLeadId});
  @override
  ConsumerState<ConsumerStatefulWidget> createState() =>
      _AddKycVerificationBodyScreenState();
}

class _AddKycVerificationBodyScreenState
    extends ConsumerState<AddKycVerificationBodyScreen> {
  @override
  Widget build(BuildContext context) {
    ref.watch(addKycVerificationScreenControllerProvider(
        existLeadId: widget.existLeadId));
    controller() => ref.read(addKycVerificationScreenControllerProvider(
            existLeadId: widget.existLeadId)
        .notifier);
  ...
  Widget generalInformationSection(
      ...
      required AddKycVerificationScreenController controller}) {
      ...
      child: Column(
        children: [
          ...
          CommonImageButton(
            ...
            errorText: controller.globalKycDetailModel.isDriverPicMandatory!
                ? controller.isSubmitOnGenInfo
                    ? controller.userUrl == ""
                        ? validateImageFile(controller.userPicFile)
                        : null
                    : null
          ...
          CommonDropdown(
            ...
            items: controller.smartPhoneUserList
                .map((lead) => lead.name ?? "")
                .toList(), // Show names in dropdown
            selectedItem: controller.smartPhoneUserList
                .firstWhere(
            ...
            errorText: controller
                    .globalKycDetailModel.isSmartphoneUserMandatory!
                ? controller.isSubmitOnGenInfo
          ...
          CommonDropdown(
            ...
            items: controller.maritalStatusList
                .map((lead) => lead.name ?? "")
                .toList(), // Show names in dropdown
            selectedItem: controller.maritalStatusList
                .firstWhere(
            ...
            border: Border(
                bottom:
                    BorderSide(color: ColorStyles().borderGray!, width: 0.5)),
            errorText: controller.globalKycDetailModel.isMaritalStatusMandatory!
                ? controller.isSubmitOnGenInfo
                  ...
                  return Theme(
                    data: Theme.of(context).copyWith(
                      datePickerTheme: DatePickerThemeData(
                        headerBackgroundColor: ColorStyles()
                            .orangePink!, // Header background color
                        headerForegroundColor:
                            Colors.white, // Header text color
                        headerHeadlineStyle: TextStyles()
                      ...
                      textButtonTheme: TextButtonThemeData(
                        style: TextButton.styleFrom(
                            ...
                            textStyle: TextStyles()
                                .robotoSize16Bold(color: ColorStyles().grey!)),
                      ),
                    ),
                    child: child!,
                  );
  ...
  Widget vehicleInformationSection(
      {required BuildContext context,
      required AddKycVerificationScreenController controller}) {
    return SingleChildScrollView(
      child: Column(
        children: [
          ...
          CommonDropdown(
            ...
            items: controller.vehicleOwnershipList
                .map((lead) => lead.name ?? "")
                .toList(), // Show names in dropdown
            selectedItem: controller.vehicleOwnershipList
                .firstWhere(
                  (lead) => lead.value == controller.selectedVehicleOwnershipId,
                  orElse: () => LeadFieldsModel(),
                )
                .name,
            dropdownDecoratorProps: DropDownDecoratorProps(
          ...
          TextInputField(
            ...
            onTap: () async {
              ...
              if (pickDate != null) {
                controller.registrationApiDateController.text =
                    pickDate.toString();
          ...
          CommonDropdown(
            ...
            items: controller.vehicleMakeList
                .map((lead) => lead.name ?? "")
                .toList(), // Show names in dropdown
            selectedItem: controller.vehicleMakeList
                .firstWhere(
  ...
  Widget bankRefInformationSection(
      {required BuildContext context,
      required AddKycVerificationScreenController controller}) {
    return SingleChildScrollView(
      child: Column(
        children: [
          ...
          CommonDropdown(
            ...
            items: controller.bankList
                .map((lead) => lead.name ?? "")
                .toList(), // Show names in dropdown
            selectedItem: controller.bankList
                .firstWhere(
          ...
          if (controller.selfieList.isNotEmpty)
            buildPhotoGrid(controller: controller),
          _verticalSizeBox(),
          if (controller.selfieList.length <= 5)
            CommonImageButton(
              ...
              // errorText: controller.isSubmitOnBankInfo
              //     ? validateMultipleImages(controller.selfieList)
              //     : null,
              errorStyle: TextStyles()
                  .robotoSize14Regular(color: ColorStyles().errorRed!)
                  .copyWith(fontSize: 12),
          ...
          CommonDropdown(
            ...
            items: controller.houseOwnershipList
                .map((lead) => lead.name ?? "")
                .toList(), // Show names in dropdown
            selectedItem: controller.houseOwnershipList
                .firstWhere(
