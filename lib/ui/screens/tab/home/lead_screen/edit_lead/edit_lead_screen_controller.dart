...
part 'edit_lead_screen_controller.g.dart';

@riverpod
class EditLeadScreenController extends _$EditLeadScreenController {
  ...
  @override
  Future<void> build({required int existLeadId}) async {
    startLoading();
...
// }
  ...
  Future<void> getLeadDetails() async {
    GetLeadDatum? getLeadDatum = await ref
        .read(leadServiceProvider)
        .getLeadDetails(leadId: selectedLeadId);

    if (getLeadDatum != null && getLeadDatum.leadId != null) {
      firstNameController.text = getLeadDatum.firstName.toString();
      lastNameController.text = getLeadDatum.lastName.toString();
      mobileNoController.text = getLeadDatum.mobileNo.toString();
      alternateNoController.text = getLeadDatum.alternateMobileNo ?? '';
      influencerReferralIdController.text = getLeadDatum.referralCode ??'';
      pinCodeController.text = getLeadDatum.pincode ?? '';


      selectLeadSource = getLeadDatum.sourceId.toString();
      selectGender = getLeadDatum.genderId.toString();
      addressController.text = getLeadDatum.address.toString();
      landMarkController.text = getLeadDatum.landmark.toString();


      selectState = getLeadDatum.stateId.toString();

      await getCityList();

      selectCity = getLeadDatum.cityId.toString();
      selectCluster = getLeadDatum.clusterId.toString();
      selectDealer = getLeadDatum.dealerId.toString();
      selectBusinessSegment = getLeadDatum.businessSegmentId.toString();
      selectVehicleType = getLeadDatum.vehicleTypeId.toString();
      selectSalesType = getLeadDatum.salesTypeId.toString();
      selectServiceType = getLeadDatum.serviceTypeId.toString();
      selectVehicleFinanceStatus =
          getLeadDatum.vehicleFinanceStatusId.toString();
      selectMoveToKyc = getLeadDatum.moveToKycVerification == true ? "Yes" : "No";
    } else {}
  }
  ...
  Future<void> leadUpdate() async {
    startLeadUpdate();
    try {
      debugPrint("Starting leadUpdate...");

      final leadModel = await ref.read(leadServiceProvider).leadUpdate(
